...
# ============================================================================
# NUOVA SEZIONE: RACCOLTA EMAIL CON GOOGLE SHEETS (ENVIRONMENT VARIABLES)
# ============================================================================
st.markdown("---")
...
with st.form("newsletter_form"):
    ...
    if submitted and email:
        try:
            # ================================================================
            # CONFIGURAZIONE GOOGLE SHEETS DA ENVIRONMENT VARIABLES (RENDER)
            # ================================================================

            # Leggi la private key e puliscila
            private_key_raw = os.getenv("GSHEETS_PRIVATE_KEY", "")

            # Rimuovi virgolette esterne se presenti (dal JSON)
            if private_key_raw.startswith('"') and private_key_raw.endswith('"'):
                private_key_raw = private_key_raw[1:-1]

            # Sostituisci \n (doppio backslash) con veri newline
            private_key = private_key_raw.replace('\\n', '\n')

            # Crea dizionario credenziali per Google Sheets
            gsheets_credentials = {
            ...
            conn = st.connection(
                ...
                service_account_info=gsheets_credentials
            )

            # ================================================================
            # LETTURA E SCRITTURA DATI SU GOOGLE SHEETS
            # ================================================================

            # Leggi i dati esistenti dal foglio "Iscrizioni"
            try:
                df_esistente = conn.read(worksheet="Iscrizioni", ttl=0)
                # Se il foglio è vuoto o non ha le colonne giuste, crea struttura
            ...
            # Verifica se l'email è già presente (evita duplicati)
            if email in df_esistente['Email'].values:
                st.warning("⚠️ Questa email è già iscritta alla newsletter!")
            else:
                # Crea nuova riga con i dati dell'iscrizione
                ...
                # Concatena con i dati esistenti
                df_aggiornato = pd.concat([df_esistente, nuova_iscrizione], ignore_index=True)

                # Scrivi i dati aggiornati su Google Sheets
            ...
            st.info("💡 Verifica che le Environment Variables su Render siano configurate correttamente")

# ============================================================================
# FINE SEZIONE GOOGLE SHEETS
# ============================================================================
